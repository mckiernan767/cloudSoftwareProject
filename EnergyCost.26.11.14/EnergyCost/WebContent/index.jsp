...
<%
	...
	System.out.println("cpu " + SimpleMain.cpuUsage.toString());
	System.out.println("energycost " + energyCost+"%");
	System.out.println("tarriff " + tarriffLevel);
%>
...
<script>     
		...
		var intialZones = <%=tarriffLevel%>.split(',');
		
        function newEnergyCost() {
            var newZone;
            if(targetStart + targetDuration/2<479)
            	newZone = 0.5;
            else if (targetStart + targetDuration/2<959)
            	newZone = 1;
            else
            	newZone = 2;
            nEnergyCost = <%=energyCost%>+(newZone - tarriffLevel[targetId])*targetCpuUsage*targetDuration/144000;
            return nEnergyCost;
...
<form name="myForm" id="main" onsubmit="return validateForm();" >		
<label>Datacentre: </label>

<select name='drivers' ><option>Please select a datacentre:</option>
	<option id="dc" value="1">  1 </option> </select> &nbsp;
 
<label>Floor: </label>

<select name='drivers' ><option>Please select a floor:</option>
	<option id="fl" value="1">  1 </option> </select> &nbsp;
        
    
<label>Rack: </label>

<select name='drivers' ><option>Please select a rack:</option>
	<option id="rc" value="1">  1 </option> </select> &nbsp;
	
<label>Host: </label>

<select name='drivers' ><option>Please select a host:</option>
	<option id="ht" value="1">  1 </option> </select> &nbsp;

 <br> <br>

    <label for="datepicker">Date: &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        <input name="date" type="text" id="datepicker" data-date-format="dd/mm/yyyy" data-date-endDate="19/11/2014" data-provide="datepicker"  />
    </label>
    <button onClick="validateForm()" > Submit</button> &nbsp; &nbsp;
    
    </form>
<form name="costForm" id="main" >     
	 <label>Initial Cost:  <input type="text" value = "<%=energyCost%>" name="initialCost" id="initialCost" readonly /></label> &nbsp; &nbsp;
     <label>New Cost:  <input type="text" name="newCost" id="newCost" readonly /></label>
     
     <button onClick="newEnergyCost()" > Calculate</button>
			...
			$(function() {
				...
				$('.dp-disable').bind(
						...
						function() {
							...
							$this.text((status ? 'Disable' : 'Enable')
									+ ' date ' + whichInput);
							this.blur();
							return false;
						});
		...
		<br> <label> Event ID:
			&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <input
			type="text" id="eventID" readonly />
		</label> <br> <label>Event start:
			&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <input type="text"
			id="eventStart" readonly />
		</label> <br> <label>Event duration: &nbsp; <input type="text"
			id="eventDuration" readonly /></label>			
		</label> <br> <label>CPU Usage: &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <input type="text"
			id="cpuUsage" readonly /></label> <br> <br>	
...
<table class="table table-bordered">
	...
	<tbody>
		...
		<%
			...
			//System.out.println("string " + ganttChartString);
			//System.out.println("cpu " + SimpleMain.cpuUsage.toString());
		%>
		...
		<script>
			...
			window.onload = function(e) {
				...
				// .Set('labels.inbar', ['Targaryen','Snow','Lannister','Stark','Baelish','Stark','Stark','Lannister'])
				.Set(
						'vbars',
				...
				//obj.set('labels.ingraph', [,null,'Hoolio',null,'Olga']);

				/**
				 * This is how to get notified once adjusting has ended
				 */
				RGraph.addCustomEventListener(gantt, 'onadjustend', function(
						obj) {
						...
						.addCustomEventListener(
								...
								function(obj) {
									...
									if (conf) {
										...
										targetId = idx;
										targetStart = events[idx][0];
										targetDuration = events[idx][1];
										targetCpuUsage = events[idx][2];
										/*Write a txt file here
										we call a java method which reads that file
										draw the chart again*/
		<%String test = request.getParameter("datepicker");
			//System.out.println("tuyfjcfyj" + test);%>
